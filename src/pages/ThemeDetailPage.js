...
const ThemeDetailPage = () => {
  const { id } = useParams(); // theme id from route
  const navigate = useNavigate();
  const { isAuthenticated } = useAuth();

  // state: theme + related data
  ...
  // user selections for plan creation
  const [selectedPackage, setSelectedPackage] = useState(null);
  const [selectedVenue, setSelectedVenue] = useState(null);
  const [eventDate, setEventDate] = useState(null);
  const [guestCount, setGuestCount] = useState(20);

  // loading state while creating plan
  const [creating, setCreating] = useState(false);

  // fetch theme + packages + venues together on mount
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // filter only packages belonging to this theme
        setPackages(packagesRes.data.filter(p => p.theme_id === parseInt(id)));

        // all venues available
        setVenues(venuesRes.data);
      } catch (error) {
        console.error('Error fetching data:', error); // debug API failure
      } finally {
  ...
  // handle plan creation flow
  const handleStartPlan = async () => {

    // auth check before allowing plan creation
    if (!isAuthenticated) {
    ...
    try {
      // payload debug log for backend verification
      ...
      console.error(error); // debug error response
    } finally {
      setCreating(false);
    }
  };
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {packages.map((pkg) => (
              <button
                ...
                onClick={() =>
                  setSelectedPackage(
                    selectedPackage?.package_id === pkg.package_id ? null : pkg
                  )
                }
              ...
              >
                ...
                <p className="font-nunito text-sm text-slate-500 mb-3">
                  {pkg.description}
                </p>
