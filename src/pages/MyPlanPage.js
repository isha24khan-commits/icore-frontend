...
/**
 * MyPlanPage Component
 * Displays the user's current event planning details
 * Allows adding/removing services and packages
 * Handles plan updates and checkout flow
 */
...
const MyPlanPage = () => {
  ...
  //selected items already in plan
  const [planServices, setPlanServices] = useState([]);
  const [planPackages, setPlanPackages] = useState([]);

  //Available options for adding to the plan
   ...
   * Also updates services and packages inside the plan
   */
  ...
  /**
   * Load plan + available services/packages on mount
  ...
  useEffect(() => {
   ...
   /**
   * Adds a service to current plan
   */
 ...
 /**
  ...
  /**
   * Updates number of guests in plan
   */
 ...
 /**
   * Creates order from plan and redirects to checkout
   */
...
// Helper arrays for filtering UI
